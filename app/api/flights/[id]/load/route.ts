...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const { seatsByCabin, r1Count, note, myidtravelStatus } = await request.json();
  ...
  // N'importe quel membre de la ligue peut mettre à jour le remplissage (ou
  // le statut MyIdTravel), pas seulement le posteur du vol.
  ...
  if (flight.data_tier === 'rich') {
    ...
    // Compagnie "basic" : pas de remplissage détaillé, seulement le statut
    // de couleur MyIdTravel comme proxy de P(embarque) (voir lib/constants.ts).
    if (!MYIDTRAVEL_STATUSES.includes(myidtravelStatus)) {
      return NextResponse.json({ error: 'Statut MyIdTravel invalide.' }, { status: 400 });
    }
    ...
    const { error } = await supabase.from('flight_load_updates').insert({
      ...
      recorded_at: recordedAt.toISOString(),
      seats_by_cabin: {},
      myidtravel_status: myidtravelStatus as MyIdTravelStatus,
      note: note || null,
    });

    if (error) {
      return NextResponse.json({ error: "Impossible d'enregistrer le statut." }, { status: 500 });
    }
  }
